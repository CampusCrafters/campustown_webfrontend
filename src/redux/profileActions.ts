...
export const fetchProfile = () => async (dispatch: Dispatch<Action<any>>) => {
    try {
        const response = await axios.get(`${backendURL}/api/v1/user/viewProfile`, { withCredentials: true });
...
export const uploadProfilePicture = (file: File) => async (dispatch: Dispatch<Action<any>>) => {
    try {
...
export const deleteProfilePicture = () => async (dispatch: Dispatch<Action<any>>) => {
    try {
        await axios.delete(`${backendURL}/api/v1/user/deleteProfilePicture`, { withCredentials: true });
        dispatch(fetchProfile() as any);
    } catch (error) {
